...
describe("Execute on parquet", () => {
  it("can do a count", async () => {
    const cw = new ConnectionWrapper();

    const rw = await cw.execute("SELECT count(*) FROM parquet_scan('src/tests/test-fixtures/alltypes_plain.parquet')");
  ...
  it("can do a select all", async () => {
    ...
    const rw = await cw.execute("SELECT * FROM parquet_scan('src/tests/test-fixtures/alltypes_plain.parquet')");
