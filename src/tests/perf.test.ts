...
describe("Perfomance test suite", () => {
  ...
  it("q1", async () => {
    const result = await connection.executeIterator(
      "SELECT http_status_code, COUNT(url), AVG(deeprank), COUNT(links_in_count), COUNT(backlink_count) FROM parquet_scan('src/tests/test-fixtures/data/20200929_093122_00057_hafyi_bucket-00000') GROUP BY http_status_code ORDER BY http_status_code",
    );
  ...
  it("q2", async () => {
    const result = await connection.executeIterator(
      "SELECT level, COUNT(url), AVG(deeprank), COUNT(links_in_count), COUNT(backlink_count) FROM parquet_scan('src/tests/test-fixtures/data/20200929_093122_00057_hafyi_bucket-00000') GROUP BY level ORDER BY level",
    );
    console.log(result.fetchAllRows());
    // expect(result.fetchAllRows()).toEqual([
    //   {
  ...
  it("q3", async () => {
    const result = await connection.executeIterator(
      "SELECT count(url) FROM parquet_scan('src/tests/test-fixtures/data/20200929_093122_00057_hafyi_bucket-00000') WHERE (http_status_code = 200 AND meta_redirect = FALSE AND primary_page = TRUE AND indexable = TRUE AND canonicalized_page = FALSE AND (paginated_page = FALSE OR (paginated_page = TRUE AND page_1 = TRUE))) AND ((css != TRUE AND js != TRUE AND is_image != TRUE AND internal = TRUE) AND (header_content_type = 'text/html' OR header_content_type = '')) ORDER BY count(url) DESC",
    );
    console.log(result.fetchAllRows());
    // expect(result.fetchAllRows()).toEqual([
    //   {
